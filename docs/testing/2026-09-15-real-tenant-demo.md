...
- 用户打开 base 落在 Bitable **自动生成的空"数据表"**上（10 条占位空行）——pulse 数据在侧边栏的任务表等 8 张表里。已通过表删除 API 删掉默认表，base 现在打开即任务表（11 条→清理后 10 条）。
- 发现同名任务"文档站搭建"×2：排查为**本会话演示命令重复执行**产生的两条真实本地任务（非同步 bug），已本地软删+远端删记录。
- REAL-2（真实网络新增）：RecordCreate **响应超时**（服务端实际成功、客户端未拿到 record_id）时，重试会重建重复记录——pending_record 机制只覆盖"响应成功但回填失败"。本次手动清理；v1.2 改进方向：create 超时后先按内容 search 核对再重试。
- sync 噪音修复：回声命中的记录不再打印"创建即定保留本地"类警告（警告推迟到回声判定之后）。
